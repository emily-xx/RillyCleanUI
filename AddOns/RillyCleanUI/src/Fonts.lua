local RillyNiceFonts=CreateFrame("Frame")
RillyNiceFonts:RegisterEvent("ADDON_LOADED")
RillyNiceFonts:SetScript("OnEvent", function(self, event)
        local FONT = "Interface\\AddOns\\RillyCleanUI\\media\\fonts\\Andika.ttf"

        if RCUIDB.damageFont == true then
          DAMAGE_TEXT_FONT            = "Interface\\Addons\\RillyCleanUI\\media\\fonts\\Bangers-Regular.ttf"
        end

        if RCUIDB.customFont == true then
