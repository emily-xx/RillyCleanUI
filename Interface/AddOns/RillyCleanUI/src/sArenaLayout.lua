...
RCSA:RegisterEvent("ADDON_LOADED")

RCSA:SetScript("OnEvent", function(self, event, addon)
    if (addon ~= 'sArena') then
        return
    end
    ...
    function layout:Initialize(frame)
        ...
        skinIcon(frame.SpecIcon, frame:GetName() .. "SpecIcon")
        skinIcon(frame.Trinket, frame:GetName() .. "TrinketIcon")
        skinIcon(frame.Racial, frame:GetName() .. "RacialIcon")
        skinIcon(frame.CastBar, frame:GetName() .. "CastBar")
    ...
    function layout:UpdateTextures(frame)
        frame.CastBar:SetStatusBarTexture(statusTexture)
        frame.HealthBar:SetStatusBarTexture(statusTexture)
        frame.PowerBar:SetStatusBarTexture(statusTexture)

        if self.db.classicBars then
            hpUnderlay:SetColorTexture(0, 0, 0, 0.5)
            ppUnderlay:SetColorTexture(0, 0, 0, 0.5)
        else
            hpUnderlay:SetTexture(statusTexture)
            ppUnderlay:SetTexture(statusTexture)
        end
    end
